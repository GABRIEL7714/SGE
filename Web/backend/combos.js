document.addEventListener('DOMContentLoaded', function () {
    ...
    const botonTexto = today < fechaInscripcion ? 'Pre Inscribirse' : 'Inscribirse';
    ...
    document.getElementById('inscribirButton').addEventListener('click', function () {
        ...
        if (!isAuthenticated) {
            ...
            let nextPage = today < fechaInscripcion ? '/pages/preinscription.html' : '/pages/categories.html';
            // Pasar el título del evento y el ID del combo seleccionado en la URL
            window.location.href = `${nextPage}?title=${encodeURIComponent(eventTitle)}&comboId=${selectedComboId}`;
        }
    });
});
